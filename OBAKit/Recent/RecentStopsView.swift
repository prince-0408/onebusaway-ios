...
private struct StopRowView: View {
        ...
        HStack(spacing: 12) {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(iconColor(for: stop.routeType).opacity(0.12))
                    .frame(width: 40, height: 40)
                Image(uiImage: Icons.transportIcon(from: stop.routeType))
                    .resizable()
                    .scaledToFit()
                    .frame(width: 20, height: 20)
                    .foregroundStyle(iconColor(for: stop.routeType))
            }
    ...
    private func iconColor(for routeType: Route.RouteType) -> Color {
        switch routeType {
        case .bus:          return .blue
        case .rail, .commuterRail: return .orange
        case .ferry:        return .cyan
        case .subway, .lightRail: return .purple
        default:            return .blue
        }
    }
}
